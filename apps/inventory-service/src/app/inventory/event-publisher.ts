...
export class InMemoryEventPublisher implements InventoryEventPublisher {
  readonly published: EventEnvelope[] = [];
...
export class RabbitMqEventPublisher implements InventoryEventPublisher {
  ...
  async publish(envelope: EventEnvelope): Promise<void> {
    ...
    try {
      ...
      RabbitMqEventPublisher.logger.warn(
        `Không thể publish sự kiện ${envelope.eventType}: ${String(error)}`,
      );
    }
  }
