...
- **Severity:** MEDIUM → **CLOSED** (2026-08-05)
- **Root cause:** `/tai-khoan/bao-hanh` only loaded/created warranty claims; title promised đổi trả but never called `/returns`.
- **Fix:** `warranty-page-inner.tsx` loads claims+returns, create flow chooses CLAIM vs RETURN with correct payloads.
- **Evidence:** UI posts to `/api/bff/warranty/returns`; list shows both kinds; ledger updated.
...
- **Severity:** MEDIUM → **CLOSED** (2026-08-05)
- **Root cause:** PDP `requireLoginOrContinue()` redirected guests before `addItem` even though CartProvider supports guest carts.
- **Fix:** `product-purchase-panel.tsx` — guest add-to-cart without login; Buy Now adds then routes to login with `next=/thanh-toan` if needed.
- **Evidence:** unit `specs/product/def-018-guest-add-to-cart.spec.tsx`; e2e `e2e/storefront/def-018-guest-add-to-cart.spec.ts`.

### DEF-019 — In-app notifications generation

- **Severity:** HIGH → **CLOSED** (2026-08-05)
- **Root cause:** `ORDER_CREATED` / `ORDER_CONFIRMED` outbox payloads carried `customerId` (in-app OK) but omitted `email`/`customerEmail`, so notification-service skipped SMTP; BFF also did not forward `x-user-email`.
- **Fix:** Order `parseActor` accepts `x-user-email`; outbox includes `email`/`customerEmail` from actor or `customerEmail` body; storefront/admin BFF forward session email; checkout may also send `customerEmail`.
- **Evidence:** unit `order.service.spec.ts` asserts outbox email; e2e `e2e/api/def-019-order-notification.spec.ts` (api project) — COD order → single `order.created` in-app row + Mailpit subject with order code (passed 2026-08-05).
...
| BLOCKER  | 0     | —                                               |
| HIGH     | 0     | —                                               |
| MEDIUM   | 3     | DEF-012 partial, DEF-013 verify, DEF-020 verify |
| LOW      | 1     | DEF-017                                         |

**Rule:** No BLOCKER or HIGH open. DEF-015, DEF-018, and DEF-019 are CLOSED.
...
| DEF-015 | 2026-08-05 | Warranty+returns UI; Playwright authenticated journeys; BFF `/returns`         |
| DEF-018 | 2026-08-05 | Guest add-to-cart; e2e `def-018-guest-add-to-cart.spec.ts` + unit source guard |
| DEF-019 | 2026-08-05 | Order outbox email + BFF x-user-email; e2e `def-019-order-notification.spec.ts` |
