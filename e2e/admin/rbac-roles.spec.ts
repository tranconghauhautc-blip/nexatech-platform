...
async function login(page: import('@playwright/test').Page, email: string) {
  ...
  await page.getByLabel(/mật khẩu/i).fill(password as string);
...
test.describe('admin 4-role login / RBAC', () => {
  ...
  for (const account of ACCOUNTS) {
    test(`${account.role} login, menu RBAC, logout`, async ({ page }) => {
      ...
      // Staff must not see Manager+ product menu
      const products = page.getByRole('link', { name: /^Sản phẩm$/i });
      ...
      const users = page.getByRole('link', { name: /Người dùng/i });
      if (account.canUsers) {
      ...
      const audit = page.getByRole('link', { name: /Nhật ký/i });
      if (account.canAudit) {
      ...
      // Route guard: Staff cannot open SuperAdmin-only users page
      if (!account.canUsers) {
  ...
  test('Admin does not get SuperAdmin-only users menu by default', async ({
    ...
    await expect(page.getByRole('link', { name: /Người dùng/i })).toHaveCount(
      0,
    );
    await page.goto('/nguoi-dung');
